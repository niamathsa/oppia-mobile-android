	...
	protected Payload doInBackground(Payload... params) {
		...
		for (Object o: payload.getData()){
			...
			try { 
				...
				System.out.println("hello i got download url task"+ m.getDownloadUrl());
				System.out.println("hello i got download url task"+ m.getDownloadUrl());
				System.out.println("hello i got download url task"+ m.getDownloadUrl());
				
				System.out.println("change into data "+ m.getDownloadUrl().replace(":83",""));
				
//				192.9.200.11/nigerian/moodle24/pluginfile.php/237/mod_page/content/3/dsdsdsdss.mp4
				URL u = new URL(m.getDownloadUrl().replace(":83",""));
...
//					payload.setResult(true); // false
					
					payload.setResult(true);
					payload.setResultResponse(ctx.getString(R.string.success_media_download,m.getFilename()));
				
//					
//					Log.d(TAG,"Error for Download url values " + resultMD5);
//					payload.setResultResponse(ctx.getString(R.string.success_media_download)); //error_media_download
				} else {
					payload.setResult(true);
					payload.setResultResponse(ctx.getString(R.string.success_media_download,m.getFilename()));
				}
